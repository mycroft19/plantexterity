...
(function () {
  ...
  // Contact + feedback forms: client-side validation with a mailto fallback.
  // If a form has a real action (not the REPLACE_ME placeholder), it submits normally.
  ["contact-form", "feedback-form"].forEach(function (id) {
    ...
    form.addEventListener("submit", function (e) {
      ...
      var msg = form.querySelector(".form-msg") || document.getElementById("form-msg");

      var emailOk = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test((email.value || "").trim());
      if (!name.value.trim() || !emailOk || !message.value.trim()) {
        e.preventDefault();
        if (msg) { msg.textContent = "Please enter your name, a valid email, and a message."; msg.style.color = "#b3261e"; }
        return;
      }

      if (form.getAttribute("action") === "REPLACE_ME") {
        e.preventDefault();
        var fSubject = form.querySelector("[name=subject]");
        var subj = encodeURIComponent((fSubject && fSubject.value) || "Website feedback");
        var body = encodeURIComponent(
          "Name: " + name.value + "\nEmail: " + email.value + "\n\n" + message.value
        );
        if (msg) { msg.textContent = "Opening your email app… (set up a form endpoint to receive these on the site)."; msg.style.color = "var(--green-dark)"; }
